import { editSectionSchema } from "@/lib/validations";
import { getGeminiClient, formatGeminiError, withRetry } from "@/lib/gemini";
import { buildEditSectionPrompt } from "@/lib/prompts";
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const { fullContent, sectionContent, instruction } = parsed.data;

    const clientApiKey = request.headers.get("x-api-key") || undefined;
    const client = getGeminiClient(clientApiKey);
    const prompt = buildEditSectionPrompt(fullContent, sectionContent, instruction);
